...
import { bookAppointmentApi, fetchBookedSlotsApi } from '@/lib/api';
...
export default function BookingWizard({ treatments = [], settings = {} }) {
  ...
  useEffect(() => {
    if (treatments.length > 0) {
      if (preselectedSlug) {
        const found = treatments.find(t => t.slug === preselectedSlug);
        if (found) {
  ...
  }, [treatments, preselectedSlug, selectedTreatment]);
          ...
          <div>
            ...
            <div className="relative mb-6">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-clinic-bg border border-clinic-border-subtle focus:border-accent focus:outline-none text-sm text-clinic-text shadow-sm"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8 max-h-[460px] overflow-y-auto pr-1">
              {filteredTreatments.length > 0 ? (
                filteredTreatments.map(item => {
                  ...
                  const catLabel = CATEGORY_MAP[item.category] || 'Clinical Procedure';
                  return (
                    <div
                      key={item.id}
                ...
                <div className="col-span-full text-center py-12 text-clinic-muted">
                  No clinical procedures match your filter criteria.
                </div>
              )}
            </div>
        ...
        {currentStep === 3 && (
          <div>
            <div className="mb-6 pb-4 border-b border-clinic-border-subtle">
              ...
              <p className="text-sm text-clinic-muted">
                Please provide your contact information to reserve your clinical session.
              </p>
            </div>
        ...
        {currentStep === 4 && bookingConfirmation && (
          <div className="text-center py-8 px-4 max-w-2xl mx-auto flex flex-col items-center">
            ...
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-primary text-accent text-sm font-semibold tracking-wide mb-6">
              <Sparkles size={16} />
              <span>Reference: #{bookingConfirmation.reference_id || `SG-${String(bookingConfirmation.id).padStart(5, '0')}`}</span>
            </div>
